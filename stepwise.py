...
class StepwisePredictor:
    ...
    def _generate_single_plan(self, step: str, step_index: int) -> str:
        """
        ...
        planning_prompt = self.prompts.get("planning", "")
        ...
        few_shot = self.prompts.get("few_shot_examples", "")
        if few_shot:
        ...
        return get_messages(self.llm, messages, self.config.get("model", {}).get("temperature", 0.2))

    def _parse_plan_into_substeps(self, plan: str) -> List[str]:
        """
    ...
    def _generate_code_for_substeps(
        self,
        step: str, 
        plan: str, 
        substeps: List[str], 
        ...
        """
        code_generation_prompt = self.prompts.get("code_generation", "")
        ...
        for i, substep in enumerate(substeps):
            if self.logger:
                self.logger.info(f"Generating code for substep {i+1}/{len(substeps)} of step {step_index+1}")
    ...
    def _combine_substeps(self, steps: List[str], substeps_code: List[str]) -> str:
        """
    ...
    def _refine_steps(self, steps: List[str], code: str) -> str:
        """
    ...
    def _combine_into_test_case(self, steps: List[str], code: str) -> str:
        """
    ...
    def _optimize_final_code(self, code: str) -> str:
        """
        ...
        """
        if self.logger:
            self.logger.info("Optimizing final code")
        ...
        messages = [
            {"role": "system", "content": "You are an expert Python developer. Optimize code for correctness, efficiency, and readability."},
        ...
        optimized_output_path = os.path.join(self.output_dir, "optimized_code.py")
