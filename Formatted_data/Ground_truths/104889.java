...
public class TC09_Zone_Operation_Lead_Role_Schedule_Task_Notification_Actions_Comment
{
    @ParameterizedTest
    @ArgumentsSource(ZoneOperationTestEnvironmentProvider.class)
    @ZoneOperationTestCaseId("104889")
    void TC_09(Map<String,String> data)
    {
        IocBuilder.execute(Duration.ofMinutes(10), EResultData.TMF_ZONE_OPERATION_LEAD_ROLE, "104889", tc ->
        {
            ...
            //STEP 5
            tc.notification.performActions(EAction.COMMENT);
            WaitFor.condition(()-> tc.modal.exists(EModal.MODAL));
            tc.stepEvaluator.reset();
            tc.stepEvaluator
                    .add(()->tc.modal.getTitle().startsWith("Comments of "),"Modal is not displayed")
                    .add(()->tc.edit.isDisplayed(EEdit.ADD_COMMENT),"'%s' Edit not found!".formatted(EEdit.ADD_COMMENT));
            tc.addStepInfo("Comment window should be reflected and user should be able to add the comment",
                    "ok",tc.stepEvaluator.eval(),new ComparerOptions().takeScreenShotPlatform());
