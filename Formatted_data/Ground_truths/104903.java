...
public class TC23_Zone_Operation_Lead_Role_View_Education_WaitList_Delete
{
    @Test
    void TC_23()
    {
        IocBuilder.execute(Duration.ofMinutes(10), EResultData.TMF_ZONE_OPERATION_LEAD_ROLE, "104903", tc ->
        {

            String notification_Number = new SimpleDateFormat("yyyyMMddHHmmssSSS").format(new Date()).substring(9);;
            String preferred_ces = "Tester11";
            ...
            //STEP 7
            //PRE MEASURES -  create a notification
            this.create_notification(tc,notification_Number,preferred_ces);
            //
            tc.table.itemClick(ETable.CAD_TABLE,EColumn.NOTIFICATION_NUMBER,notification_Number);
            ...
            //STEP 11
            tc.table.itemClick(ETable.CAD_TABLE,EColumn.byIndex(0),"#0");
            ...
            tc.addStepInfo("window Popup message should be displayed as \"\n" +
                    ...
                    true , tc.modal.getTitle().equals("Delete") && tc.button.isDisplayed(EButton.DELETE),new ComparerOptions().takeScreenShotPlatform());
            ...
            tc.spinner.waitForSpinnerToDisappear(ESpinner.TABLE_LOADER,Duration.ofSeconds(10));
            tc.stepEvaluator
                    .add(()-> tc.table.getRowsCount(ETable.CAD_TABLE)==1,"Notification has not been deleted");
            tc.addStepInfo("Selected Notification should be deleted",
                    "ok",tc.stepEvaluator.eval(),new ComparerOptions().takeScreenShotPlatform());
        });
    }
