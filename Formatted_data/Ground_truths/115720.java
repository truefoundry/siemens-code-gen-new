...
public class TC05_Dispatcher_Education_Role_Schedule_Task_Notification_Actions_Change_Status
{
    @ParameterizedTest
    @ArgumentsSource(EducationTestEnvironmentProvider.class)
    @EducationTestCaseId("115720")
    void TC_05(Map<String,String> data)
    {
        IocBuilder.execute(Duration.ofMinutes(10), EResultData.TMF_DISPATCHER_EDUCATION_ROLE, "115720", tc ->
        {
            ...
            //STEP 4
            tc.notification.performActions(EAction.CHANGE_STATUS_DISPATCHED);
            WaitFor.specificTime(Duration.ofSeconds(1));
            tc.addStepInfo("Depending on Notification status should be reflected like Open, Open-Waitlist, Confirmed, Dispatched",
                    true, tc.notification.getStatus().equals("Dispatched"),
                    new ComparerOptions().takeScreenShotPlatform());
        });
    }
}
