...
public class TC25_Zone_Operation_Lead_Role_Views_ResourceTerritory_Refresh
{
    @Test
    void TC_25()
    {
        IocBuilder.execute(Duration.ofMinutes(10), EResultData.TMF_ZONE_OPERATION_LEAD_ROLE, "104905", tc ->
        {
            ...
            tc.stepEvaluator
                    ...
                    .add(()-> tc.table.exists(ETable.CAD_TABLE), "No Datas are found in the table ");
            tc.addStepInfo("Resource territory view screen should be reflected with Search Bar, Manage Filter, Sorting, Refresh,New,Export buttons and list of resource territory list item",
                    "ok", tc.stepEvaluator.eval(),new ComparerOptions().takeScreenShotPlatform());

            //STEP 5
            tc.button.click(EButton.REFRESH);
            tc.addStepInfo("Refresh options should be have dropdown list like Refresh every time, 5 minutes, 10 minutes and 15 minutes",
                    true,tc.combo.getDropDownItems(EComboBox.REFRESH).containsAll(List.of("5 Minutes","10 Minutes","15 Minutes")),new ComparerOptions().takeScreenShotPlatform());
