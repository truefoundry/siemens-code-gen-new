...
public class TC11_Zone_Operation_Lead_Role_Schedule_ClipBoard_Notification_Actions_GetCandidates
{
    @ParameterizedTest
    @ArgumentsSource(ZoneOperationTestEnvironmentProvider.class)
    @ZoneOperationTestCaseId("104891")
    void TC_11(Map<String,String> data)
    {
        IocBuilder.execute(Duration.ofMinutes(10), EResultData.TMF_ZONE_OPERATION_LEAD_ROLE, "104891", tc ->
        {
            ...
            //STEP 5
            tc.notification.performActions(EAction.GET_CANDIDATES);
            ...
            tc.addStepInfo("Rules Violation Window should be reflected with close button",
                    true,tc.modal.getTitle().equals("No Candidates Found"),
                    new ComparerOptions().takeScreenShotPlatform());
