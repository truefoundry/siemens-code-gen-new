...
public class TC08_Zone_Operation_Lead_Role_Schedule_Task_Notification_Actions_PinTask
{
    @ParameterizedTest
    @ArgumentsSource(ZoneOperationTestEnvironmentProvider.class)
    @ZoneOperationTestCaseId("104888")
    void TC_08(Map<String,String> data)
    {
        IocBuilder.execute(Duration.ofMinutes(10), EResultData.TMF_ZONE_OPERATION_LEAD_ROLE, "104888", tc ->
        {
            ...
            //STEP 5
            tc.notification.performActions(EAction.PIN_TASK);
            WaitFor.specificTime(Duration.ofSeconds(5));
            tc.addStepInfo("The task selected should be pinned",
                    true,tc.notification.isPinned("#0"),new ComparerOptions().takeScreenShotPlatform());
            ...
            try
            {
                ...
                tc.notification.performActions(EAction.UNPIN_TASK);
                WaitFor.specificTime(Duration.ofSeconds(3));
            }
            catch (Exception e)
            {
                TcLog.error("There was an error while unpin the task"+e);
            }
        });
    }
}
