...
public class TC_ID_4007
{
    @Test
    void tc_rs_1897()
    {
        IocBuilder.execute(Duration.ofMinutes(30), EResultData.TEST_AUTOMATION, "4007", tc ->
        {
...
//            int adjustedUdp0 = TestUtils.getUdpFromColumn(tc, ETable.byIndex(0), EColumn.byCustomValue(currentMonth));
            ...
            if(!assumptionsCurrentMonthNumber.contains("#not found!"))
            {
                ...
                WaitFor.condition(()-> tc.tab.isSelected(ETab.ASSUMPTION_DETAILS), Duration.ofSeconds(5));
            ...
            if(!assumptionsCurrentMonthPlus1Number.contains("#not found!"))
            {
                ...
                WaitFor.specificTime(Duration.ofSeconds(2));
                List<String> assumptionsCurrentMonthPlus1 = tc.table.getColumnItems(ETable.byIndex(1),
                        EColumn.ASSUMPTION);
                tc.stepEvaluator
                        .add(() -> !assumptionsCurrentMonthPlus1.isEmpty(),
                                "Assumption details not visible(%s)!".formatted(currentMonthPlus1));
            }
