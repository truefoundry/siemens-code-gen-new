...
public class TC29_Dispatcher_Education_Role_Schedule_ClipBoard_Notification_Actions_BookAppointment
{
    @ParameterizedTest
    @ArgumentsSource(EducationTestEnvironmentProvider.class)
    @EducationTestCaseId("115744")
    void TC29(Map<String,String> data)
    {
        IocBuilder.execute(Duration.ofMinutes(8), EResultData.TMF_DISPATCHER_EDUCATION_ROLE, "115744", tc ->
        {
            final String notification = data.get("notification");
            tc.browser.start(WebDrv.CHROME, ETestData.URL, ETestData.DISPATCHER_EDUCATION_ROLE_USER);
