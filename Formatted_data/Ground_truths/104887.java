...
public class TC07_Zone_Operation_Lead_Role_Schedule_Task_Notification_Actions_ShowRelatedTasks
{
    @ParameterizedTest
    @ArgumentsSource(ZoneOperationTestEnvironmentProvider.class)
    @ZoneOperationTestCaseId("104887")
    void TC_07(Map<String,String> data)
    {
        IocBuilder.execute(Duration.ofMinutes(10), EResultData.TMF_ZONE_OPERATION_LEAD_ROLE, "104887", tc ->
        {
            ...
            String expanded_Notification = tc.notification.getAllNotification_Names().get(0);
            ...
            //STEP 5
            tc.notification.performActions(EAction.SHOW_RELATED_TASK);
            tc.tab.select(ETab.CLIPBOARD);
            tc.addStepInfo("Task related to the same notification should be reflected",
                    true,tc.notification.getAllNotification_Names().stream().allMatch(names -> names.equals(expanded_Notification))
                    ,new ComparerOptions().takeScreenShotPlatform());
        });
    }
}
