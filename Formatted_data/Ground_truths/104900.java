...
public class TC20_Zone_Operation_Lead_Role_Views_CapacityUsedDaily_CapacityUsedDailyListItem
{
    @Test
    void TC_20()
    {
        IocBuilder.execute(Duration.ofMinutes(10), EResultData.TMF_ZONE_OPERATION_LEAD_ROLE, "104900", tc ->
        {
            ...
            tc.stepEvaluator
                    .add(()-> tc.browser.getCurrentURL().toLowerCase().contains("CapacityLimitsDynamic".toLowerCase()), "Capacity used daily Page is not visible")
                    ...
                    .add(()-> tc.table.getRowsCount(ETable.CAD_TABLE) > 0, "Capacity used daily list is empty");
            tc.addStepInfo("Periodic Capacity screen should be reflected with Search Bar, Manage Filter, Sorting," +
                            " Refresh,New,Export buttons and list of capacity used daily",
            ...
            tc.stepEvaluator
                    ...
                    .add(()-> tc.edit.getAllLabelNames().containsAll(List.of("Name:","District:","Task Type Category:",
                            "Time Resolution:","Entry Date:","Capacity Threshold:","Actual Capacity Used:")),
                            "The labels that should be present under capacity limits dynamic is not present");
            tc.addStepInfo("Detail screen should be reflected for the selected list item with Name,District,Task " +
                            "Type category, Time Resolution,Capacity Threshold fields and OK, Apply, Cancel button",
                    "ok", tc.stepEvaluator.eval(),new ComparerOptions().takeScreenShotPlatform());
