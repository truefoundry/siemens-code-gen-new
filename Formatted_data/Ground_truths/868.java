...
public class TC04_Internal_User_Question_About_An_Order_Or_eSupport_Assistance
{
    @Test
    void Question_About_An_Order_Or_eSupport_Assistance()
    {
        IocBuilder.execute(Duration.ofMinutes(20), EResultData.INTERNAL_USER, "868", tc ->
        {
            ...
            //Step 1
            tc.browser.start(WebDrv.CHROME, ETestData.QA_ENV_URL, new CoreStartOptions().startIncognito(), new CoreStartOptions());
            ...
            WaitFor.condition(() -> tc.edit.getValue(EEdit.SOLD_TO).equalsIgnoreCase(soldTo));
            tc.stepEvaluator
                    .add(() -> tc.edit.getValue(EEdit.REASON).equals(reason), "reason is not editable")
                    .add(() -> tc.edit.getValue(EEdit.SOLD_TO).equals(soldTo), "Sold to is not editable")
                    .add(() -> tc.edit.getValue(EEdit.SHIP_TO).equals(shipTo), "Ship to is not editable")
                    .add(() -> tc.edit.getValue(EEdit.MOBILE).equals(number), "Mobile is not editable");
            tc.addStepInfo("Values of the fields are the same as were entered in step 3 and 5", "ok", tc.stepEvaluator.eval()
                    , new ComparerOptions().takeScreenShotPlatform());
